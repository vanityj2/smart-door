...
//                printf("cjson 内存大小为 = %d\r\n",sizeof(cjson)); // 打印JSON对象的内存大小
                // 解析数据
                if(cJSON_GetObjectItem(json_data,"\"door\"")!=NULL) // 如果存在LEDSwitch键
                {
                    Door_state = cJSON_GetObjectItem(json_data,"\"door\"")->valueint; // 提取LEDSwitch的值
//                    printf("csjon解析成功 Door = %d\r\n",Door_state); // 打印LEDSwitch的值
